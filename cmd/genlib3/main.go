...
var (
	gopath, tensorLoc, execLoc, denseLoc, stdengLoc, floatengLoc string
)
...
func genFloatEngineUnOpMethods(w io.Writer) {
	for _, op := range floatengUnOps {
		floatEngUnOp.Execute(w, op)
	}
}
...
func pipeline(pkg, filename string, fns ...func(io.Writer)) {
	...
	// for floatengLoc, we also specify import of math32
	if pkg == floatengLoc {
		fmt.Fprint(f, `import (
